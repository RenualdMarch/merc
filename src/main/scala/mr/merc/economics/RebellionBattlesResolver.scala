...
class RebellionBattlesResolver(state: WorldState) {
  ...
  def rebellions(rebellions:List[Rebellion]):Map[Rebellion, Battle] = {
    rebellions.map { reb =>
      ...
      val newRebelState = state.generateNewState(reb.rebellionCulture, mostPopularParty, reb.province.owner.technologyLevel.technologyLevel,  0, reb.province.name)
      reb -> new RebellionOneProvinceBattle(state.worldHexField.buildTerrainHexField(state.seasonOfYear.season),
        reb.province, newRebelState, reb.province.regionWarriors.allWarriors, state.turn)
    }.toMap
  }
}
