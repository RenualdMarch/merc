...
class TargetInfoTest extends FunSuite {
  ...
  test("killing possibility is better that without it") {
    val better = agent.TargetInfo(field.hex(0, 0), field.hex(0, 1), AttackResultPrediction(0.1, 0.7, 10, 10))
    val worse = agent.TargetInfo(field.hex(0, 0), field.hex(1, 0), AttackResultPrediction(0.1, 0.6, 10, 10))
  ...
  test("when possible to kill both enemies, must choose those who is more expensive") {
    val better = agent.TargetInfo(field.hex(0, 0), field.hex(1, 0), AttackResultPrediction(0.1, 0.8, 10, 10))
    val worse = agent.TargetInfo(field.hex(0, 0), field.hex(0, 1), AttackResultPrediction(0.1, 0.8, 10, 10))
    assert(better > worse === true)
    assert(worse < better === true)
  }

  test("when not possible to kill, must choose those where damage done/received ratio is bigger") {
    val better = agent.TargetInfo(field.hex(0, 0), field.hex(1, 0), AttackResultPrediction(0.1, 0.5, 5, 10))
    val worse = agent.TargetInfo(field.hex(0, 0), field.hex(0, 1), AttackResultPrediction(0.1, 0.5, 7, 10))
    assert(better > worse === true)
    assert(worse < better === true)
  ...
  test("prev test can work with zeros") {
    val better = agent.TargetInfo(field.hex(0, 0), field.hex(1, 0), AttackResultPrediction(0.1, 0.5, 0, 10))
    val worse = agent.TargetInfo(field.hex(0, 0), field.hex(0, 1), AttackResultPrediction(0.1, 0.5, 0, 5))
    assert(better > worse === true)
    assert(worse < better === true)
  }
}