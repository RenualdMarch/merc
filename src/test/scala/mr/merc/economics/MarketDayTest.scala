...
class MarketDayTest extends FunSuite {
  ...
  test("oversupply"){
    val marketDay = new MarketDay(Products.Coal, 10)
    ...
    assert(marketDay.fulfilledSupply.get.toSet === Set(FulfilledSupplyRequest(200, supplyRequest1),
       FulfilledSupplyRequest(50, supplyRequest2)))
  ...
  test("overdemand") {
    ...
    assert(marketDay.fulfilledSupply.get.toSet === Set(FulfilledSupplyRequest(100, supplyRequest1),
       FulfilledSupplyRequest(200, supplyRequest2)))
    assert(marketDay.tomorrowPrice.get > 10)
  }
  ...
  test("equal") {
    ...
    assert(marketDay.fulfilledSupply.get.toSet === Set(FulfilledSupplyRequest(400, supplyRequest1),
      FulfilledSupplyRequest(200, supplyRequest2)))
    assert(marketDay.tomorrowPrice.get === 10)
  }
  ...
  test("no demand") {
    ...
    marketDay.acceptRequests(request)
    marketDay.calculateSupplyAndDemand()
    assert(marketDay.fulfilledSupply.get === List(FulfilledSupplyRequest(0, request)))
    assert(marketDay.fulfilledDemands.get.isEmpty)
  }
